...
import type { ISigner } from '../stores/types/pure'
...
type LegitChainIds = Extract<keyof typeof definedContracts, ChainId>
type LegitContracts = Exclude<keyof typeof definedContracts, ChainId>
...
export function useContracts<SelectedChainId extends LegitChainIds>(
  ...
  if (
    definedContracts.hasOwnProperty(toMainChainIds(chainId ?? config.DEFAULT_CHAINID))
  ) {
    for (const [key, value] of Object.entries<ContractFunction<ChainId>>(allContracts)) {
      const contract = value(chainId ?? config.DEFAULT_CHAINID)
      if (signer) {
