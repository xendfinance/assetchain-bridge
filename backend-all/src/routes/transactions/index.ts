...
import { GetUserTransactionDto, AddTransactionDto, MarkTransactionAsClaimedDto } from '@/types'
// import axios from 'axios'

export default (): Resource => ({
  async get(req: Request<{}, {}, {}, any>, res) {
    try {
      let {chainIds, fulfilled,limit, page, userAddress} = req.query
      if (!userAddress) {
      ...
      const response = await bridgeService.getUserTransactions(userAddress, options, chainIds, fulfilled)
      res.status(200).json({
        success: true,
        data: response,
      })
  ...
  async put(req: Request<{}, {}, MarkTransactionAsClaimedDto>, res) {
    try {
      ...
      if (!transactionId || !claimTransactionHash || !toBridgeId) {
        return res.status(400).json({ error: 'Missing required fields: transactionId, claimTransactionHash, toBridgeId' })
      }
