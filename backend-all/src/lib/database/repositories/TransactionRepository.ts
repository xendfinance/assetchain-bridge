...
export class TransactionRepository {
  ...
  async findByFulfillFromUserAndFulfillNonce(fulfillFromUser: string, fulfillNonce: number) {
    return this.repository.findOne({ 
  ...
  async updateByFromUserAndNonce(fromUser: string, nonce: number, transactionData: Partial<Transaction>) {
    await this.repository.update({ fromUser, nonce }, transactionData);
    return this.findByFromUserAndNonce(fromUser, nonce);
  }

  async updateByFulfillFromUserAndFulfillNonce(fulfillFromUser: string, fulfillNonce: number, transactionData: Partial<Transaction>) {
  ...
  async updateByTransactionHash(transactionHash: string, transactionData: Partial<Transaction>) {
  ...
  async deleteByFulfillFromUserAndFulfillNonce(fulfillFromUser: string, fulfillNonce: number) {
  ...
  async findTransactionsByAmountRange(minAmount: string, maxAmount: string) {
    return this.repository
      .createQueryBuilder("transaction")
      .where("CAST(transaction.amount AS NUMERIC) >= CAST(:minAmount AS NUMERIC)", { minAmount })
      .andWhere("CAST(transaction.amount AS NUMERIC) <= CAST(:maxAmount AS NUMERIC)", { maxAmount })
      .getMany();
  }
  ...
  async existsByFulfillFromUserAndFulfillNonce(fulfillFromUser: string, fulfillNonce: number) {
    const count = await this.repository.count({ where: { fulfillFromUser, fulfillNonce } });
    return count > 0;
  }
  ...
  async findTransactionsByUserWithPagination(userAddress: string, offset: number, limit: number) {
    return this.repository.find({
      where: { userAddress },
      skip: offset,
      take: limit,
