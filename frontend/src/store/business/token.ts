...
export const useTokenRead = () => {
  ...
  return {
    balanceToken: (chainId: ChainId) => {
      return token.balances[chainId]?.formatString(token.decimals, 18) ?? '0'
    },
    balanceTokenNumber: (chainId: ChainId) => {
      return token.balances[chainId]?.formatNumber(token.decimals, 18) ?? 0
    },
    balanceNative: (chainId: ChainId) => {
      return token.balancesNative[chainId]?.formatNumber(token.decimals, 5) ?? 0
    },
    ...
    getCurrentDecimals: (tokenAddres: string) => computed(() => token.cDecimals[tokenAddres]),
