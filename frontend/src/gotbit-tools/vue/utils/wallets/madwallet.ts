...
export class MadWallet extends WalletHandler {
  constructor(
    ...
    if (!this.nativeProvider) {
      alert('Please set up Mad Wallet properly')
      throw new Error('Please set up Mad Wallet properly')
    }
  ...
  async connect(): Promise<boolean> {
    try {
      ...
      if (+(error as any).code == 4001) {
        alert('Please connect to Mad Wallet.')
      } else {
        alert('Please connect to Mad Wallet.')
        console.error(error)
      }
